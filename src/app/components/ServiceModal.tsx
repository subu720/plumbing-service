      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 flex items-start gap-4">
            {service.imageUrl ? (
              <div className="w-28 h-28 rounded-xl overflow-hidden flex-shrink-0 bg-slate-50">
                <img src={service.imageUrl} alt={service.imageLabel ?? service.title} className="w-full h-full object-cover" />
              </div>
            ) : (
              <div className="w-28 h-28 rounded-xl flex-shrink-0 bg-gradient-to-br from-cyan-50 to-sky-50 flex items-center justify-center text-slate-700">
